...
class BlackPawnPiece {
  possibleMoves(piece) {
    ...
    const downSquare = document.getElementById(down);
    if (downSquare.children.length < 1) validMoves.push(down);
    ...
    possibleAttacks.push(downLeft, downRight);
    ...
    const validAttacks = possibleAttacks.filter(pos => {
      ...
      if (occupyingPiece &&
        occupyingPiece.classList.contains('white')) return pos;
    });
...
export default new BlackPawnPiece();